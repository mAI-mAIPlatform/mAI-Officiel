...
export default function QuizzlySocialPage() {
  ...
  const [social, setSocial] = useState<SocialState>({ friends: [], friendRequests: ["Leo92", "NinaMath"] });
            ...
            <div className="rounded-2xl border border-slate-100 p-4">
              ...
              <div className="mt-3 grid sm:grid-cols-2 gap-2">
                ...
                {social.friends.map((friend) => (
                  <div key={friend} className="bg-violet-50 text-violet-700 px-3 py-2 rounded-lg flex items-center justify-between">
                    <span>{friend}</span>
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50">
                {messages.length === 0 ? (
                  ...
                  messages.map((m, i) => (
                    <div key={`${m}-${i}`} className={`p-3 rounded-xl max-w-[70%] ${m.startsWith("Moi") ? "bg-violet-600 text-white ml-auto" : "bg-white border border-slate-200 text-slate-700"}`}>{m}</div>
                  ))
                )}
              </div>
